...
def extract_stock(categories: list, stock: list):
    book_categories = {category: 0 for category in categories}

    for book in stock:
        book_info = book.split(" ")
        category = book_info[0][0]

        if category in book_categories.keys():
            book_categories[category] += int(book_info[1])

    return [(category, quantity) for category, quantity in book_categories.items()]
